...
public class SpectrumPhaseGLPanel extends GLCanvas implements GLEventListener {

    private double[] freq;// = new double[512];
    private double[] amp;
    ...
    public SpectrumPhaseGLPanel(GLCapabilities caps, int gbl_sweep_point) {
        super(caps);
        //System.out.println("length selected"+" "+gbl_sweep_point);
        for(int i=0;i<gbl_sweep_point;i++)
        {
        	this.freq[i] = i;
        }
...
//        }
    ...
    // UPDATE SPECTRUM
    // ------------------------
    public void updateSpectrum( double[] a) {
       
        this.amp = a;

        minFreq = this.freq [0];
        maxFreq = this.freq [this.freq .length - 1];
...
//        gl.glPopMatrix();
        ...
        for (double f = minFreq; f <= maxFreq; f += step) {
            ...
            String line1 = String.valueOf((int)f); // frequency value
